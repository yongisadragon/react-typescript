...
// 2. 무슨 타입이 들어올지 확신이 있을때 씀. as number라고 해놓고 변수는 유니온으로, 그런다음 함수 인자로 '123'이렇게 문자열 넣어도 as number해놔도 버그 캐치못함. 즉, 정말 필요할때 버그 추적 못함. 비상용 혹은 디버깅용임. 웬만하면 if문을 쓰셈.
function 내함수4(x: number | string) {
  let arr: number[] = [];
  arr[0] = x as number; //이딴식으로 하면 ts가 x를 number로 인식해줌
}
내함수4(3);
